...
async def audit(seed):
    root = Path(f"runs/c5_hover_repeat_20260914_s{seed}")
    ...
    events = [
        json.loads(line) for line in (root / "events.jsonl").read_text(encoding="utf-8").splitlines()
    ]
    calls = {
        c["observation_seq"]: c
...
async def main():
    ...
    (OUT / "DEPARTURE_AUDIT.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    for run in result:
        print("seed", run["seed"])
