...
for seed in freeze["seeds"]:
    name = f"c5_hover_stable_20260914_s{seed}"
    root = Path("runs") / name
    manifest = json.loads((root / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["environment_config"] == reference["environment_config"]
    assert manifest["architecture_config"]["id"] == freeze["architecture"]
...
proof = {
    ...
    "environment_matches_historical": True,
    "new_flights": rows,
