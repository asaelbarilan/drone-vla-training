...
root = Path("reports/vla_balanced_comparison_20260917")
...
models = [m for m in ("existing_control", "expanded") if (root / f"{m}_report.json").exists()]
buf = io.BytesIO()
...
with sync_playwright() as p:
    ...
    page.goto("http://127.0.0.1:8771/balanced_comparison.html")
    page.get_by_text("Exact model text prompt", exact=True).click()
    assert page.locator("#model option").count() == len(models)
    reference = json.loads((root / "existing_control_report.json").read_text())
    review_ids = manifest["generation_eval_ids"] + reference["extra_eval_ids"]
    assert page.locator("#sample option").count() == len(review_ids)
    for model in models:
        report = json.loads((root / f"{model}_report.json").read_text())
        report["after"] += report["extra_after"]
        ...
        for phase in ("before", "after", "blank_image", "blank_instruction"):
            ...
            for r in records:
                index = review_ids.index(r["decision_id"])
                ...
                if (
                    phase == "after"
                    and row["task_group"] == "visual"
                    and execution_file.exists()
                    and r["seed"] < 1450
                ):
    ...
    page.locator("#model").select_option("existing_control")
    ...
    assert (
        ...
        == (root / "validation_loss_curves.png").read_bytes()
    )
    ...
    assert (
        ...
        == (root / "task_loss_curves.png").read_bytes()
    )
    ...
    assert (
        ...
        == (root / "training_loss_curves.png").read_bytes()
    )
    assert page.evaluate("document.documentElement.scrollWidth<=innerWidth")
    assert not errors
    browser.close()
