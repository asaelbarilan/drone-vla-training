...
from train_uav_flow_vla import OFFICIAL_REPORT, encode, load_image, official_rows, official_stats
...
def main():
    ...
    flights = sorted(by_flight)[: args.flights]
    ...
    def act(row, mirror):
        ...
        batch = encode(processor, tokenizer, row, args.chunk, with_answer=False, image=image)
        ...
        return tokenizer.decode(ids, args.chunk)

    records, changed, flipped_y, flipped_yaw, unparsed = [], 0, 0, 0, 0
    for flight in flights:
        steps = sorted(by_flight[flight], key=lambda r: r["step"])
        picks = np.linspace(0, len(steps) - 2, args.frames).astype(int)
        for index in picks:
            row = steps[int(index)]
            real, mirrored = act(row, False), act(row, True)
    ...
    (args.out_dir / f"mirror_{args.tag}.json").write_text(
