"""Rebuild D-109 repetition comparison from stored controls, without inference."""
...
async def main():
    ...
    for name in NAMES:
        ...
        if name.startswith("c5_hover_repeat_20260914_"):
    ...
    banner = (
        "D-109: two unchanged repetitions of the D-108 level-approach hover variant. "
        "Same geometry; this checks repeatability, not novel-scene generalization. "
        "Historical seed-1061 success is included for comparison."
    )
    page = page.replace(
        "<body>",
    ...
    Path("reports/debugger/hover_repeat.html").write_text(page, encoding="utf-8")
