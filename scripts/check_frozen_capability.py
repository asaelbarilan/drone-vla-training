"""Check actual D-106 replay pages in Edge, without model calls."""
...
with sync_playwright() as p:
    ...
    for family in args.families:
        out = Path("reports/frozen_capability_20260913/browser") / family
        ...
        for i in range(count):
            ...
            for t in [0, 10, 60]:
                ...
                assert (
                    "NOT architecture performance" not in page.locator("#taskStatus").inner_text()
                )
        ...
        print(
            f"PASS {family}: {len(checks)} snapshots, {count} playback checks, no browser errors",
            flush=True,
        )
    browser.close()
