...
NAMES=("c1_visual_tool_search_20260913_s1061", "c1_coordinate_completion_20260913_s1061", "c1_visual_tool_refined_20260913_s1061",
       "c1_visual_tool_visible_20260913_s1061", "capability_screen_c1_known_goal_20260912_s1061",
...
async def main():
    ...
    for name in NAMES:
        ...
        runs.append(run)
        result=run["result"]
        summary.append({"run":name,"success":result["success"],"reason":result["termination_reason"],
            "sim_s":result["sim_duration_s"],"wall_s":result["wall_duration_s"],
            "metrics":result["metrics"],"replay":proof,
            "calls":[{k:c.get(k) for k in ("id","role","image_count","status","observation_seq","started_t_sim_ns","completed_t_sim_ns")} for c in run["recordings"]]})
        if "20260913" in name:
            dest=OUT/name
            dest.mkdir(exist_ok=True)
            for filename in ("manifest.json","result.json"):
                (dest/filename).write_bytes((path/filename).read_bytes())
            (dest/"events.jsonl.gz").write_bytes(gzip.compress((path/"events.jsonl").read_bytes(),mtime=0))
    ...
    Path("reports/debugger/aerialclaw_tools.html").write_text(html_document({"schema":1,"runs":runs,"sources":_sources()}),encoding="utf-8")
    (OUT/"RUNS.json").write_text(json.dumps(summary,indent=2),encoding="utf-8")
    print("PASS: replay trajectories; new flights",[(r["run"],r["success"]) for r in summary[:4]],flush=True)
