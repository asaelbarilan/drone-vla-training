...
def cells():
    ...
    for family in ("c0", "c1", "c5"):
        for scenario in SCENARIOS:
            ...
            profile = {"c0": "c0", "c1": "c1_capability_gemma_dev", "c5": "c5_capability_gemma_dev"}[family]
            if visual:
                profile = "c1_visual_search_gemma_dev"
            result.append(dict(family=family, scenario=scenario, profile=profile,
                environment=f"capability_{scenario}" + ("_tools" if visual else ""),
                name=f"frozen_{family}_{scenario}_20260913_s1061",
                integration_limited=family == "c1" and scenario not in ("known_goal", "visible_target", "turn_search")))
...
def resolved(cell):
    return dict(architecture=load_architecture(cell["profile"]).model_dump(mode="json"),
...
async def main():
    ...
    if args.stage == "freeze":
        ...
        frozen=dict(source_commit=subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),
            hashes=snapshot(), cells=[{**c, **resolved(c)} for c in matrix])
    ...
    for cell in selected:
        ...
        arch=load_architecture(cell["profile"]); env=load_environment(cell["environment"])
        write_manifest(build_manifest(arch,env,seeds=[1061]),dest)
        print("START",cell["name"],flush=True)
        runtime=Orchestrator(arch,env,EpisodeSpec(episode_id=cell["name"],seed=1061),out_dir=dest,debug_capture=True)
        result=await runtime.run()
        data=result.model_dump(mode="json")
        (dest/"result.json").write_text(json.dumps(data,indent=2),encoding="utf-8")
        evidence=OUT/cell["name"]; evidence.mkdir(exist_ok=True)
        for filename in ("manifest.json","result.json"):
            (evidence/filename).write_bytes((dest/filename).read_bytes())
        print(json.dumps(dict(run=cell["name"],success=data["success"],termination=data["termination_reason"],
            sim_s=data["sim_duration_s"],wall_s=data["wall_duration_s"],error=data.get("error"))),flush=True)
        if data.get("error"):
