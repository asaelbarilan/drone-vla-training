...
async def analyze(run_dir: Path) -> dict:
    ...
    yaw_rows: list[dict] = []
    plans = [e for e in events if e["event_type"] == "plan" and e["payload"].get("feasible")]
    plan_idx = 0
    active_goal = None
    rows: list[dict] = []
    ...
    for event in controls:
        ...
        while plan_idx < len(plans) and plans[plan_idx]["t_sim_ns"] <= event["t_sim_ns"]:
            metadata = plans[plan_idx]["payload"].get("planner_metadata", {})
            if metadata.get("semantic_goal_xyz"):
                active_goal = np.array([float(v) for v in metadata["semantic_goal_xyz"].split(",")])
            plan_idx += 1
        pos = np.array([observation.position.x, observation.position.y, observation.position.z])
        command = event["payload"]
        moving = math.hypot(command["vx"], command["vy"]) > 1e-6
        direction = math.atan2(command["vy"], command["vx"]) if moving else None
        semantic = math.atan2(*(active_goal - pos)[1::-1]) if active_goal is not None else None
        truth_delta = env.goal - pos
        truth_bearing = math.atan2(truth_delta[1], truth_delta[0])
        yaw_rows.append(
            {
                "t": event["t_sim_ns"] / 1e9,
                "yaw_deg": math.degrees(observation.yaw_rad),
                "visible": current_bbox is not None,
                "distance_m": status.distance_to_goal_m,
                "target_relative_deg": math.degrees(
                    _wrap_angle(truth_bearing - observation.yaw_rad)
                ),
                "semantic_relative_deg": math.degrees(_wrap_angle(semantic - observation.yaw_rad))
                if semantic is not None
                else None,
                "motion_relative_deg": math.degrees(_wrap_angle(direction - observation.yaw_rad))
                if moving
                else None,
                "motion_semantic_difference_deg": math.degrees(_wrap_angle(direction - semantic))
                if moving and semantic is not None
                else None,
                "semantic_target_difference_deg": math.degrees(
                    _wrap_angle(semantic - truth_bearing)
                )
                if semantic is not None
                else None,
                "yaw_rate": command["yaw_rate"],
                "safety_modified": command.get("safety_modified", False),
            }
        )
        ...
        if decision is not None and observation.rgb is not None:
            ...
            if bbox is not None:
                ...
                row["predicted_inside_target_bbox"] = (
                    x0 <= predicted_u <= x1 and y0 <= predicted_v <= y1
                )
