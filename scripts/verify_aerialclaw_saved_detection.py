...
async def main():
    ...
    mission=MissionSpec(mission_id="saved-probe",instruction=config.instruction,
        task_family=config.task_family,success=config.params["success"],
        constraints=config.params["constraints"],allowed_skills=config.params["allowed_skills"])
    ...
    assert np.array_equal(np.asarray(saved),np.asarray(rgb)), "Saved frame differs; cannot reuse geometry"
    assert not obs.semantic_hits and obs.privileged is None
    ctx=DecisionContext(mission=mission,observation=obs,
        ...
        t_sim_ns=obs.t_sim_ns,t_wall_ns=0,episode_id="saved-probe")
    response=json.loads((root/"debug/calls/call-000002.json").read_text(encoding="utf-8"))["response"]
    ...
    report={"same_saved_rgb_pixels":True,"image_sha256":hashlib.sha256(saved.tobytes()).hexdigest(),
        "response":json.loads(response),"unrefined_position":None,
        "refined_position":refined.model_dump(),"measured_depth_m":d,
        "refinement_fraction":0.03,"new_model_calls":0}
    Path("reports/aerialclaw_tools_20260913/SAVED_FRAME_GEOMETRY.json").write_text(json.dumps(report,indent=2),encoding="utf-8")
