...
root = Path("reports/vla_smol_mixed_20260917")
...
models = [m for m in ("smol256", "qwen") if (root / f"{m}_mixed_report.json").exists()]
buf = io.BytesIO()
...
with sync_playwright() as p:
    ...
    for model in models:
        report = json.loads((root / f"{model}_mixed_report.json").read_text())
