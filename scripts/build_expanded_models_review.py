...
table('scores',['Model','All','Seen','Unseen','Macro recall','False STOP'],Object.entries(DATA.summary.official_transfer).map(([k,v])=>[k,`${v.all.correct}/${v.all.n}`,`${v.seen.correct}/${v.seen.n}`,`${v.unseen.correct}/${v.unseen.n}`,(100*v.all.macro_recall).toFixed(1)+'%',`${v.all.false_stop}/${v.all.nonterminal}`]));
