...
PAGE = r"""<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Joint openfly_vla data + Local — three models</title>
...
<h1>Joint openfly_vla data + Local · 256M / 500M / Qwen</h1><p>110 openfly_vla data source routes, 88 train / 22 validation. 2,929 native + 1,156 local training examples. Eight inconsistent source actions excluded. Both sources appear in every effective batch.</p><p class="muted">Bounded local pilots: 400 updates/model, fresh adapters, sequential GPU jobs. No AWS. openfly_vla data: 3m / 30° navigation primitives; local: velocity-bin JSON. These are separate, explicit instruction modes. openfly_vla data validation below is an offline diagnostic, not a flown benchmark.</p>
<p><a href="openfly_same_panel.html">Compare released openfly_vla with our models on the same72cases; full dataset size</a></p><section><strong id="status">Loading live status…</strong><p id="time" class="muted"></p><div id="models"></div></section>
<section><label>Model <select id="model"><option>smol256</option><option>smol500</option><option>qwen</option></select></label><h2>Loss curves</h2><div class="grid"><div><h3>Optimization loss</h3><div id="trainplot"></div><p class="muted">20-update moving average, separately by source. Changing examples and oversampled rare actions make individual steps noisy.</p></div><div><h3>Fixed-probe loss</h3><div id="valplot"></div><p class="muted">Identical examples, evaluation mode, at 0/100/200/300/400 updates. Solid: validation; dashed: training. Blue: local; orange: openfly_vla data. Lower is better within the same model/source.</p></div></div><div id="metrics"></div><div id="flights"></div></section>
<section><h2>Held-out observations and actual model output</h2><select id="case"></select><div id="images" style="margin-top:18px"></div><p id="target"></p><div id="outputs"></div><details><summary>Instruction and source provenance</summary><pre id="provenance"></pre></details><p class="muted">For openfly_vla data, three recorded views run oldest → current; the label refers to the current view's next primitive. These are source observations, not model-generated flight frames. Local commands are in the observation forward/right/down frame, applied for 0.2 seconds. STOP ends the mission; it is not a landing command. Negative right means left; negative down means up.</p></section>
...
$('model').onchange=render;$('case').onchange=showCase;fetch('joint_openfly_cases.json').then(r=>r.json()).then(c=>{cases=c;$('case').innerHTML=c.map((x,i)=>`<option value="${i}">${esc((x.source==='openfly'?'openfly_vla data':x.source)+' · '+x.group+' · '+x.id.slice(-90))}</option>`).join('');showCase();});refresh();setInterval(refresh,10000);
</script></html>"""
