"""Rebuild D-110 repetition comparison from stored controls, without inference."""
...
async def main():
    ...
    for name in NAMES:
        ...
        if name.startswith("c5_hover_stable_20260914_"):
    ...
    banner = (
        "D-110: fixed hover reference, live completion and residual heading hold. "
        "Same geometry; this checks repeatability, not novel-scene generalization. "
        "Historical seed-1062 departure failure is included for comparison."
    )
    page = page.replace(
        "<body>",
    ...
    Path("reports/debugger/hover_stable.html").write_text(page, encoding="utf-8")
