...
Uses frozen validation seeds1400/1405 and one training-scene diagnostic1401.
"""
...
async def rollout(model, processor, data, out, seed, adapter, wall_deadline):
    ...
    for tick in range(200):  # ten simulated seconds, at most50 actual generations
        ...
        if tick % 4 == 0:
            ...
            call = {
                ...
                "prompt": prompt,
                "response": raw,
                "response_schema": {"contract": "direct_velocity_yaw_level_v1"},
            ...
            emit(
                ...
                {
                    "role": "policy",
                    "model_id": "Qwen3-VL-4B-Instruct+local-r8-LoRA",
...
async def main(args):
    ...
    model = PeftModel.from_pretrained(load_base(), str(args.adapter), is_trainable=False).eval()
    processor = AutoProcessor.from_pretrained(str(MODEL), local_files_only=True)
    results, runs = [], []
    deadline = time.monotonic() + 900
    for seed in (1400, 1405, 1401):
        if time.monotonic() >= deadline:
            break
        folder, result = await rollout(
            model, processor, args.data, args.out, seed, args.adapter, deadline
        )
        ...
        runs.append(run)
        write_json(
            args.out / "summary.json", {"results": results, "real_time_capable_claim": False}
        )
