"""Rebuild D-111 repetition comparison from stored controls, without inference."""
...
async def main():
    ...
    banner = (
        "D-111: original clutter seed1061 with corrected depth and navigation-compatible C5 fixes. "
        "Historical corrected-depth run is a comparator, not an isolated yaw ablation. "
        "Inspect source images, proposed points, planner output and actual movement."
    )
    page = page.replace(
        "<body>",
    ...
    Path("reports/debugger/clutter_stable.html").write_text(page, encoding="utf-8")
