...
async def rollout(model, processor, data, out, seed, adapter, wall_deadline, mode):
    ...
    name = f"{MODEL_TAG}_frd_{mode}_s{seed}"
    folder = out / name
    (folder / "debug/calls").mkdir(parents=True, exist_ok=False)
    (folder / "images").mkdir()
    arch = load_architecture("c7").model_copy(
        update={
            "id": f"{MODEL_TAG}_frd_{mode}_offline_diagnostic",
            "name": f"{MODEL_TAG} {mode} offline FRD actions",
        }
    )
        ...
        events.append(
            {
                ...
                "component": f"{MODEL_TAG}_offline_diagnostic",
                "payload": payload,
                "trace_id": trace,
            }
        )
    ...
    for tick in range(200):  # ten simulated seconds, at most50 actual generations
        ...
        if tick % 4 == 0:
            ...
            emit(
                ...
                {
                    "role": "policy",
                    "model_id": f"{MODEL_TAG}/{mode}/FRD",
                    "latency_ms": latency * 1000,
                },
                decision_id,
            )
            if error:
                # Rejected raw output is a debugger event, never a router action.
                emit(
                    "decision_proposed",
                    obs,
                    {
                        "producer": f"actual_{MODEL_TAG}_frd_{mode}",
